...
namespace COPTimerJob.Importador
{
    public class Documento
    {
        ...
        public Documento(string consulta, string titulo, string consultor, string pregunta, string respuesta, string fecha, string carpeta)
        {
        ...
        public void CrearDocumento(string pathDocumentos)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(pathDocumentos + this._consulta + ".docx", WordprocessingDocumentType.Document))
            {
                ...
                FileStream stylesTemplate =
                    new FileStream(@"F:\Estilos\styles.xml", FileMode.Open, FileAccess.Read);                    
