...
using System.Data;
using COPTimerJob.Log;

namespace COPTimerJob.Importador
{
    public class GeneradorDocumentos
    {
        ...
        public GeneradorDocumentos(string db, string connectionString, string pathDocs, bool delDia)
        {
            tipoConsulta = delDia;
            path = pathDocs;
        ...
        private void RecorrerReader(DbDataReader dbReader)
        {
            while (dbReader.Read())
            {
                CrearDocumento(dbReader["consulta"].ToString(), dbReader["titulo"].ToString(), dbReader["consultor"].ToString(), dbReader["pregunta"].ToString(), dbReader["respuesta"].ToString(), RefactorFecha((DateTime)dbReader["fecha"]), ObtenerCarpeta(dbReader));
            }
        }
        ...
        private void CrearDocumento(string consulta, string titulo, string consultor, string pregunta, string respuesta, string fecha, string carpeta)
        {
            try
            {
                Documento documento = new Documento(consulta, titulo, consultor, pregunta, respuesta, fecha, carpeta);
