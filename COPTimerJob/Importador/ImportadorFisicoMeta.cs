...
namespace COPTimerJob.Importador
{
    public class ImportadorFisicoMeta
    {
        ...
        public ImportadorFisicoMeta(string path, string siteUrl, string db, string connectionString)
        {
            _path = path;
            _url = siteUrl;
            _db = db;
                ...
                using (SPSite site = new SPSite(_url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        ...
                        try
                        {
                            ...
                            StreamWriter sw = new StreamWriter(_path + "\\" + "fechaConsulta.txt", true);
                            foreach (string archivo in archivos)
                            {
                                ...
                                {
                                    sw.Close();
                                    StreamReader sr = new StreamReader(_path + "\\" + "fechaConsulta.txt");
                                    ...
                                    sr.Close();
                                    sw = new StreamWriter(_path + "\\" + "fechaConsulta.txt", true);
                                }
                            }
                            sw.Close();
                        }
